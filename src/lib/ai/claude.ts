// Claude AI integration for WorthGoing enrichment
...
- description: a clean one-sentence description of THIS promo only — what the fan gets / does / experiences for this specific item. Pull ONLY from text adjacent to this specific date AND this specific promo. NEVER mention or reference other promos on the same date (don't say "the t-shirt plus the $3 concessions" — those are separate entries). NEVER repeat the promo name in its own description ("Free T-Shirt Night: Free T-shirt for…" is wrong; just describe the experience). Card UI stacks all of a date's promos in separate rows, so cross-references make the card read repetitively. Each description must stand alone.
